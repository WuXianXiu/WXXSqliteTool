...
+ (BOOL)createTable:(Class)cls uid:(NSString *)uid {
    ...
    NSString *createTableSql = [NSString stringWithFormat:@"create table if not exists %@(%@, primary key(%@))", tableName, [WXXDataModelTool columnNamesAndTypesStr:cls], primaryKey];
    
    
    // 2. 执行
    return [WXXSqliteTool deal:createTableSql uid:uid];
...
+ (BOOL)isTableRequiredUpdate:(Class)cls uid:(NSString *)uid {
    ...
    NSArray *modelNames = [WXXDataModelTool allTableSortedIvarNames:cls];
    
    // 2. 获取当前表格, 所有字段名称, 并排序
    NSArray *tableNames = [WXXDataTableTool tableSortedColumnNames:cls uid:uid];
...
+ (BOOL)updateTable:(Class)cls uid:(NSString *)uid {
    ...
    // 1. 创建一个拥有正确结构的临时表
    // 1.1 获取表格名称
    NSString *tmpTableName = [WXXDataModelTool tmpTableName:cls];
    NSString *tableName = [WXXDataModelTool tableName:cls];
    ...
    NSString *createTableSql = [NSString stringWithFormat:@"create table if not exists %@(%@, primary key(%@));", tmpTableName, [WXXDataModelTool columnNamesAndTypesStr:cls], primaryKey];
    ...
    NSArray *oldNames = [WXXDataTableTool tableSortedColumnNames:cls uid:uid];
    NSArray *newNames = [WXXDataModelTool allTableSortedIvarNames:cls];
    ...
    return [WXXSqliteTool dealSqls:execSqls uid:uid];
...
+ (BOOL)saveOrUpdateModel:(id)model uid:(NSString *)uid {
    ...
    // 1. 判断表格是否存在, 不存在, 则创建
    if (![WXXDataTableTool isTableExists:cls uid:uid]) {
    ...
    NSArray *result = [WXXSqliteTool querySql:checkSql uid:uid];
    
    
    // 获取字段名称数组
    NSArray *columnNames = [WXXDataModelTool classIvarNameTypeDic:cls].allKeys;
    ...
    return [WXXSqliteTool deal:execSql uid:uid];
}
...
+ (BOOL)deleteModel:(id)model uid:(NSString *)uid {
    ...
    return [WXXSqliteTool deal:deleteSql uid:uid];
...
+ (NSArray *)queryAllModels:(Class)cls uid:(NSString *)uid {
    ...
    // xx 字符串
    // oo 字符串
    NSArray <NSDictionary *>*results = [WXXSqliteTool querySql:sql uid:uid];
...
+ (NSArray *)parseResults:(NSArray <NSDictionary *>*)results withClass:(Class)cls {
    ...
    // 属性名称 -> 类型 dic
    NSDictionary *nameTypeDic = [WXXDataModelTool classIvarNameTypeDic:cls];
